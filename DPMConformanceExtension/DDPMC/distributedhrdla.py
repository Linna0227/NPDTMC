...
from decompose_conformance_method import hrdecomposetreela
import time
import pm4py
...
def covertToXlog(traces):
    ...
    for traceString in traces:
        ...
        if len(eventnames[1:]) == 1 and eventnames[1:] == ['']:
            log.append(trace)
        else:
...
def cost_token(partitiondatas):
    ...
    for ilog in partitiondatas:
        ...
        for one_trace in variant_traces:
            confor_net = get_method(trace_allocate_method).get_model(one_trace, m_net)
...
if __name__=="__main__":
    ...
    trace_allocate_method = TraceAllocateVariants.VERSION_ACTIVITY_SET_IMPROVE
    r_path = r"/home/hadoop/Projects/exresult/Alayer_actset.xlsx"
    s_path = "/home/hadoop/Projects/exresult/Alayer_actset.xlsx"
    ...
    # print(best_worse_cost)
    


    d_start_time=time.time()
    root = tree._get_root()
    sub_trees_list = hrdecomposetreela.apply(root,Dthreshold)
